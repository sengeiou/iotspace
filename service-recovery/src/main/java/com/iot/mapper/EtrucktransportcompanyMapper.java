package com.iot.mapper;

import com.iot.bean.Etrucktransportcompany;
import com.iot.bean.Etrucktransportcompanyv;
import com.iot.util.RedisCache;
import org.apache.ibatis.annotations.*;
import org.apache.ibatis.type.JdbcType;

import java.util.List;

@CacheNamespace(implementation = RedisCache.class)
public interface EtrucktransportcompanyMapper {
    @Select({
            "${sql}"
    })
    List<Etrucktransportcompany> select(@Param("sql") String sql);

    @Select({
            "select",
            "id, truckid, submituserid, confirmuserid, plasticnumber, plasticweight, glassnumber, glassweight, time, status, note, transportcompanyid",
            "from etrucktransportcompany"
    })
    @Results({
            @Result(column="id", property="id", jdbcType=JdbcType.INTEGER, id=true),
            @Result(column="truckid", property="truckid", jdbcType=JdbcType.INTEGER),
            @Result(column="submituserid", property="submituserid", jdbcType=JdbcType.INTEGER),
            @Result(column="confirmuserid", property="confirmuserid", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticnumber", property="plasticnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticweight", property="plasticweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="glassnumber", property="glassnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="glassweight", property="glassweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="time", property="time", jdbcType=JdbcType.TIMESTAMP),
            @Result(column="status", property="status", jdbcType=JdbcType.VARCHAR),
            @Result(column="note", property="note", jdbcType=JdbcType.VARCHAR),
            @Result(column="transportcompanyid", property="transportcompanyid", jdbcType=JdbcType.INTEGER)
    })
    List<Etrucktransportcompany> selectAll();

    @Select({
            "select",
            "id, truckid, submituserid, confirmuserid, plasticnumber, plasticweight, glassnumber, glassweight, time, status, note, transportcompanyid",
            "from etrucktransportcompany",
            "where ${sql}"
    })
    @Results({
            @Result(column="id", property="id", jdbcType=JdbcType.INTEGER, id=true),
            @Result(column="truckid", property="truckid", jdbcType=JdbcType.INTEGER),
            @Result(column="submituserid", property="submituserid", jdbcType=JdbcType.INTEGER),
            @Result(column="confirmuserid", property="confirmuserid", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticnumber", property="plasticnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticweight", property="plasticweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="glassnumber", property="glassnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="glassweight", property="glassweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="time", property="time", jdbcType=JdbcType.TIMESTAMP),
            @Result(column="status", property="status", jdbcType=JdbcType.VARCHAR),
            @Result(column="note", property="note", jdbcType=JdbcType.VARCHAR),
            @Result(column="transportcompanyid", property="transportcompanyid", jdbcType=JdbcType.INTEGER)
    })
    List<Etrucktransportcompany> selectBySql(@Param("sql") String sql);

    @Select({
            "select",
            "id, truckid, submituserid, confirmuserid, plasticnumber, plasticweight, glassnumber, glassweight, time, status, note, transportcompanyid",
            "from etrucktransportcompany",
            "where id = #{id,jdbcType=INTEGER}"
    })
    @Results({
            @Result(column="id", property="id", jdbcType=JdbcType.INTEGER, id=true),
            @Result(column="truckid", property="truckid", jdbcType=JdbcType.INTEGER),
            @Result(column="submituserid", property="submituserid", jdbcType=JdbcType.INTEGER),
            @Result(column="confirmuserid", property="confirmuserid", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticnumber", property="plasticnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticweight", property="plasticweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="glassnumber", property="glassnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="glassweight", property="glassweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="time", property="time", jdbcType=JdbcType.TIMESTAMP),
            @Result(column="status", property="status", jdbcType=JdbcType.VARCHAR),
            @Result(column="note", property="note", jdbcType=JdbcType.VARCHAR),
            @Result(column="transportcompanyid", property="transportcompanyid", jdbcType=JdbcType.INTEGER)
    })
    Etrucktransportcompany selectByPrimaryKey(Integer id);

    @Select({
            "${sql}"
    })
    List<Etrucktransportcompanyv> selectV(@Param("sql") String sql);

    @Select({
            "select",
            "id, truckid, submituserid, confirmuserid, plasticnumber, plasticweight, glassnumber, glassweight, time, status, note, transportcompanyid",
            "from etrucktransportcompany"
    })
    @Results(value = {
            @Result(column="id", property="id", jdbcType=JdbcType.INTEGER, id=true),
            @Result(column="truckid", property="truckid", jdbcType=JdbcType.INTEGER),
            @Result(column="submituserid", property="submituserid", jdbcType=JdbcType.INTEGER),
            @Result(column="confirmuserid", property="confirmuserid", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticnumber", property="plasticnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticweight", property="plasticweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="glassnumber", property="glassnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="glassweight", property="glassweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="time", property="time", jdbcType=JdbcType.TIMESTAMP),
            @Result(column="status", property="status", jdbcType=JdbcType.VARCHAR),
            @Result(column="note", property="note", jdbcType=JdbcType.VARCHAR),
            @Result(column="transportcompanyid", property="transportcompanyid", jdbcType=JdbcType.INTEGER),
            @Result(column = "truckid", property = "truck",
                    one = @One(select = "com.iot.mapper.EtruckMapper.selectVByPrimaryKey")),
            @Result(column = "submituserid", property = "submitUser",
                    one = @One(select = "com.iot.mapper.EuserMapper.selectByPrimaryKey")),
            @Result(column = "confirmuserid", property = "confirmUser",
                    one = @One(select = "com.iot.mapper.EuserMapper.selectByPrimaryKey")),
            @Result(column = "transportcompanyid", property = "transportcompany",
                    one = @One(select = "com.iot.mapper.EtransportcompanyMapper.selectByPrimaryKey"))
    })
    List<Etrucktransportcompanyv> selectVAll();

    @Select({
            "select",
            "id, truckid, submituserid, confirmuserid, plasticnumber, plasticweight, glassnumber, glassweight, time, status, note, transportcompanyid",
            "from etrucktransportcompany",
            "where ${sql}"
    })
    @Results({
            @Result(column="id", property="id", jdbcType=JdbcType.INTEGER, id=true),
            @Result(column="truckid", property="truckid", jdbcType=JdbcType.INTEGER),
            @Result(column="submituserid", property="submituserid", jdbcType=JdbcType.INTEGER),
            @Result(column="confirmuserid", property="confirmuserid", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticnumber", property="plasticnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticweight", property="plasticweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="glassnumber", property="glassnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="glassweight", property="glassweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="time", property="time", jdbcType=JdbcType.TIMESTAMP),
            @Result(column="status", property="status", jdbcType=JdbcType.VARCHAR),
            @Result(column="note", property="note", jdbcType=JdbcType.VARCHAR),
            @Result(column="transportcompanyid", property="transportcompanyid", jdbcType=JdbcType.INTEGER),
            @Result(column = "truckid", property = "truck",
                    one = @One(select = "com.iot.mapper.EtruckMapper.selectVByPrimaryKey")),
            @Result(column = "submituserid", property = "submitUser",
                    one = @One(select = "com.iot.mapper.EuserMapper.selectByPrimaryKey")),
            @Result(column = "confirmuserid", property = "confirmUser",
                    one = @One(select = "com.iot.mapper.EuserMapper.selectByPrimaryKey")),
            @Result(column = "transportcompanyid", property = "transportcompany",
                    one = @One(select = "com.iot.mapper.EtransportcompanyMapper.selectByPrimaryKey"))
    })
    List<Etrucktransportcompanyv> selectVBySql(@Param("sql") String sql);

    @Select({
            "select",
            "id, truckid, submituserid, confirmuserid, plasticnumber, plasticweight, glassnumber, glassweight, time, status, note, transportcompanyid",
            "from etrucktransportcompany",
            "where id = #{id,jdbcType=INTEGER}"
    })
    @Results({
            @Result(column="id", property="id", jdbcType=JdbcType.INTEGER, id=true),
            @Result(column="truckid", property="truckid", jdbcType=JdbcType.INTEGER),
            @Result(column="submituserid", property="submituserid", jdbcType=JdbcType.INTEGER),
            @Result(column="confirmuserid", property="confirmuserid", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticnumber", property="plasticnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticweight", property="plasticweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="glassnumber", property="glassnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="glassweight", property="glassweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="time", property="time", jdbcType=JdbcType.TIMESTAMP),
            @Result(column="status", property="status", jdbcType=JdbcType.VARCHAR),
            @Result(column="note", property="note", jdbcType=JdbcType.VARCHAR),
            @Result(column="transportcompanyid", property="transportcompanyid", jdbcType=JdbcType.INTEGER),
            @Result(column = "truckid", property = "truck",
                    one = @One(select = "com.iot.mapper.EtruckMapper.selectVByPrimaryKey")),
            @Result(column = "submituserid", property = "submitUser",
                    one = @One(select = "com.iot.mapper.EuserMapper.selectByPrimaryKey")),
            @Result(column = "confirmuserid", property = "confirmUser",
                    one = @One(select = "com.iot.mapper.EuserMapper.selectByPrimaryKey")),
            @Result(column = "transportcompanyid", property = "transportcompany",
                    one = @One(select = "com.iot.mapper.EtransportcompanyMapper.selectByPrimaryKey"))
    })
    Etrucktransportcompanyv selectVByPrimaryKey(Integer id);

    @Insert({
        "insert into etrucktransportcompany (id, truckid, submituserid, ",
        "confirmuserid, plasticnumber, plasticweight, glassnumber, glassweight, time,  status, ",
        "transportcompanyid, note)",
        "values (#{id,jdbcType=INTEGER}, #{truckid,jdbcType=INTEGER}, ",
        "#{submituserid,jdbcType=INTEGER}, ",
        "#{confirmuserid,jdbcType=INTEGER}, #{plasticnumber,jdbcType=INTEGER}, #{plasticweight,jdbcType=DOUBLE}, #{glassnumber,jdbcType=INTEGER}, #{glassweight,jdbcType=DOUBLE}, #{time,jdbcType=TIMESTAMP}, ",
        "#{status,jdbcType=VARCHAR}, #{transportcompanyid,jdbcType=INTEGER}, #{note,jdbcType=VARCHAR})"
    })
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(Etrucktransportcompany record);

    @Update({
        "update etrucktransportcompany",
        "set truckid = #{truckid,jdbcType=INTEGER},",
          "submituserid = #{submituserid,jdbcType=INTEGER},",
          "confirmuserid = #{confirmuserid,jdbcType=INTEGER},",
            "plasticnumber = #{plasticnumber,jdbcType=INTEGER},",
            "plasticweight = #{plasticweight,jdbcType=DOUBLE},",
            "glassnumber = #{glassnumber,jdbcType=INTEGER},",
            "glassweight = #{glassweight,jdbcType=DOUBLE},",
            "transportcompanyid = #{transportcompanyid,jdbcType=INTEGER},",
          "time = #{time,jdbcType=TIMESTAMP},",
          "status = #{status,jdbcType=VARCHAR},",
          "note = #{note,jdbcType=VARCHAR}",
        "where id = #{id,jdbcType=INTEGER}"
    })
    int update(Etrucktransportcompany record);

    @Delete({
            "delete from etrucktransportcompany",
            "where id = #{id,jdbcType=INTEGER}"
    })
    int delete(Integer id);

    @Delete({
            "delete from etrucktransportcompany",
            "where id in ${ids}"
    })
    int deletes(@Param("ids") String ids);

    @Select({
            "select t2.id id, t2.truckid truckid, t2.submituserid submituserid, t2.confirmuserid confirmuserid,",
            "t2.plasticnumber plasticnumber, t2.plasticweight plasticweight,",
            "t2.glassnumber glassnumber, t2.glassweight glassweight, t2.time time, t2.status status, t2.note note, t2.transportcompanyid transportcompanyid",
            "from etruck t1, etrucktransportcompany t2",
            "where ${sql}"
    })
    @Results({
            @Result(column="id", property="id", jdbcType=JdbcType.INTEGER, id=true),
            @Result(column="truckid", property="truckid", jdbcType=JdbcType.INTEGER),
            @Result(column="submituserid", property="submituserid", jdbcType=JdbcType.INTEGER),
            @Result(column="confirmuserid", property="confirmuserid", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticnumber", property="plasticnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="plasticweight", property="plasticweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="glassnumber", property="glassnumber", jdbcType=JdbcType.INTEGER),
            @Result(column="glassweight", property="glassweight", jdbcType=JdbcType.DOUBLE),
            @Result(column="time", property="time", jdbcType=JdbcType.TIMESTAMP),
            @Result(column="status", property="status", jdbcType=JdbcType.VARCHAR),
            @Result(column="note", property="note", jdbcType=JdbcType.VARCHAR),
            @Result(column="transportcompanyid", property="transportcompanyid", jdbcType=JdbcType.INTEGER),
            @Result(column = "truckid", property = "truck",
                    one = @One(select = "com.iot.mapper.EtruckMapper.selectVByPrimaryKey")),
            @Result(column = "submituserid", property = "submitUser",
                    one = @One(select = "com.iot.mapper.EuserMapper.selectByPrimaryKey")),
            @Result(column = "confirmuserid", property = "confirmUser",
                    one = @One(select = "com.iot.mapper.EuserMapper.selectByPrimaryKey")),
            @Result(column = "transportcompanyid", property = "transportcompany",
                    one = @One(select = "com.iot.mapper.EtransportcompanyMapper.selectByPrimaryKey"))
    })
    List<Etrucktransportcompanyv> selectVByStatusAndSql(@Param("sql") String sql);
}