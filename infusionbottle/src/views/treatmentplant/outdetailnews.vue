<template>
    <div id="yudingBox">
        ...
        <div class="tableBox">
          <p style="height:30px;font-size: 14px">出库明细：</p>
            ...
            <div class="pafinationBox">
              <div>
                  <el-button type="primary" @click="clickAddOrUp">添加明细</el-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        addForm:{
            ...
            trucktreatmentplantreceiveid: this.$route.query.id,
        },
        //表格数据
        tableData: [],
        tableTypes: [],
    ...
    methods: {
      ...
      getSelectList(){
        var _this = this;
        var obj = {"id":'',"name":"全部"};
        this.$ajax.get("/recovery/category/selectByTreatmentplantid",{
          params:{
            treatmentplantid: sessionStorage.getItem('belongId'),
          }
      ...
      getData(){
        ...
        this.$ajax.get("/recovery/outboundrecord/selectVByPage",{
            params:{
              page: this.page,
              rows: this.pageSize,
              treatmentplantid: sessionStorage.getItem('belongId'),
      ...
      handleEdit(index, row) {
        ...
        that.addForm = {
            ...
            trucktreatmentplantreceiveid: row.trucktreatmentplantreceiveid,
            time: row.time
        } 
      },
      // 新增
      addSaveForm(index){
        ...
        if(this.addForm.categoryid == null){
          this.$message.error("*为必填项！");
          return;
        }
        var that = this;
        var params = this.$qs.stringify(this.addForm, { indices: false })
        if(this.addOrUpTit == "添加明细"){
          this.$ajax.post("/recovery/outboundrecord/insert",params).then(function(response){
            if(response.data.status == 260){
          ...
          this.$ajax.put("/recovery/outboundrecord/update",params).then(function(response){
            if(response.data.status == 260){
              that.centerDialogVisible = false;
      ...
      delAll(){
        ...
        this.$confirm('此操作将永久删除选中的明细, 是否继续?', '提示', {
          ...
          var that = this;
          this.$ajax.delete("/recovery/outboundrecord/deletes",{data:this.multipleSelection}).then(function(response){
            if(response.data.status == 260){
      ...
      handleDelete(index, row) {
        this.$confirm('此操作将删除该明细, 是否继续?', '提示', {
          ...
          var that = this;
          this.$ajax.delete("/recovery/outboundrecord/delete",{params:{
            id: row
          }}).then(function(response){
            if(response.data.status == 260){
              that.$message.success("删除成功")
