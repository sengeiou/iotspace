    ...
    <div id="yudingBox">
        <div class="selectBox">
          <el-form ref="form" :model="form" label-width="60px">
            <el-form-item label="破碎料类别" label-width="90px">
                <el-select v-model="form.type" placeholder="请选择破碎料类别" @change="selectList">
        ...
        <div class="tableBox">
            ...
            <div class="pafinationBox">
              <div>
                  <!-- <el-button type="primary" @click="clickAddOrUp">添加破碎料</el-button> -->
          ...
          <div class="dialogMain">
            <el-form ref="addForm" :model="addForm" label-width="100px">
                <el-form-item label="破碎料类别">
                    <el-select v-model="addForm.cmaterialcategoryid" placeholder="请选择破碎料类别">
                        <el-option v-for="(item,index) in tableTypes" :key="index" v-show="item.name != '全部'"  :label="item.name" :value="item.id"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="去向">
                    <el-select v-model="addForm.source" placeholder="请选择破碎料去向">
                        <el-option v-for="(item,index) in sourceList" :key="index" v-show="item.text != '全部'"  :label="item.text" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="重量（kg）">
                    <el-input v-model="addForm.weight" placeholder="请输入破碎料重量"></el-input>
                </el-form-item>
                <el-form-item label="数量（袋）">
                    <el-input v-model="addForm.number" placeholder="请输入破碎料数量"></el-input>
                </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      getSelectList(){
        ...
        //出库去向
        this.$ajax.get("/recovery/cmaterialout/sourceList",{
          params:{
            treatmentplantid: sessionStorage.getItem('belongId'),
          }
        }).then(function(response){
      ...
      getData(){
        ...
        this.$ajax.get("/recovery/cmaterialout/selectVByPage",{
            params:{
              page: this.page,
              rows: this.pageSize,
              treatmentplantid: sessionStorage.getItem('belongId'),
        ...
        // this.$ajax.get("/recovery/cmaterialout/sumWeightGroupByCategory",{
        //     params:{
        //       treatmentplantid: sessionStorage.getItem('belongId'),
      ...
      addSaveForm(index){
        ...
        if(this.addOrUpTit == "添加破碎料"){
          this.$ajax.post("/recovery/cmaterialout/insert",params).then(function(response){
            if(response.data.status == 260){
          ...
          this.$ajax.put("/recovery/cmaterialout/update",params).then(function(response){
            if(response.data.status == 260){
              that.centerDialogVisible = false;
      ...
      delAll(){
        ...
        this.$confirm('此操作将永久删除选中的破碎料, 是否继续?', '提示', {
          ...
          var that = this;
          this.$ajax.delete("/recovery/cmaterialout/deletes",{data:this.multipleSelection}).then(function(response){
            if(response.data.status == 260){
      ...
      handleDelete(index, row) {
        this.$confirm('此操作将删除该破碎料, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          ...
          var that = this;
          this.$ajax.delete("/recovery/cmaterialout/delete",{params:{
            id: row
          }}).then(function(response){
            if(response.data.status == 260){
              that.$message.success("删除成功")
