    ...
    <div id="yudingBox">
        ...
        <div class="tableBox">
            <el-table :data="tableData" style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="name" label="颗粒料类别"></el-table-column>
            ...
            <div class="pafinationBox">
              <div>
                  <el-button type="primary" @click="clickAddOrUp">创建颗粒料类别</el-button>
        ...
        <el-dialog
          ...
          <div class="dialogMain">
            <el-form ref="addForm" :model="addForm" label-width="100px">
                <el-form-item label="颗粒料类别" required>
...
<script>
  export default {
    ...
    methods: {
      ...
      getData(){
        var that = this;
        this.$ajax.get("/recovery/gmaterialcategory/selectByPage",{
            params:{
              page: this.page,
              rows: this.pageSize,
              treatmentplantid: sessionStorage.getItem('belongId'),
      ...
      addSaveForm(index){
        ...
        if(this.addOrUpTit == "创建颗粒料类别"){
          this.$ajax.post("/recovery/gmaterialcategory/insert",params).then(function(response){
            if(response.data.status == 260){
          ...
          this.$ajax.put("/recovery/gmaterialcategory/update",params).then(function(response){
            if(response.data.status == 260){
              that.centerDialogVisible = false;
      ...
      delAll(){
        ...
        this.$confirm('此操作将永久删除选中的颗粒料类别, 是否继续?', '提示', {
          ...
          var that = this;
          this.$ajax.delete("/recovery/gmaterialcategory/deletes",{data:this.multipleSelection}).then(function(response){
            if(response.data.status == 260){
      ...
      handleDelete(index, row) {
        this.$confirm('此操作将删除该颗粒料类别, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          ...
          var that = this;
          this.$ajax.delete("/recovery/gmaterialcategory/delete",{params:{
            id: row
          }}).then(function(response){
            if(response.data.status == 260){
              that.$message.success("删除成功")
