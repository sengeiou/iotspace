    ...
    <div id="yudingBox">
        <div class="selectBox">
          <el-form ref="form" :model="form" label-width="65px">
            ...
            <el-form-item label="角色" v-if="userType == '医疗机构' || userType == '回收利用企业' || userType == '再利用企业' || userType == '回收物流'">
        ...
        <el-dialog
          ...
          <div class="dialogMain">
            <el-form ref="form" :model="addForm" label-width="100px">
              <el-form-item label="角色" required v-if="userType == '医疗机构' || userType == '回收利用企业' || userType == '再利用企业' || userType == '回收物流'">
                <el-select placeholder="请选择用户角色" v-model="addForm.role">
                  <el-option v-for="(item,index) in roleList" :key="index" v-show="item.text != '全部'" :label="item.text" :value="item.value"></el-option>
                </el-select>
              </el-form-item>
              ...
              <!-- <el-form-item label="类型" required>
                <el-select placeholder="请选择用户类型" v-model="addForm.type" @change="typeChange(addForm.type)">
                  <el-option v-for="(item,index) in tableTypes" :key="index" v-show="item.text != '全部'" :label="item.text" :value="item.value"></el-option>
                </el-select>
              </el-form-item> -->
              <!-- <el-form-item label="所属单位" required>
                <el-select v-model="addForm.belongid" placeholder="请选择单位">
                  <el-option v-for="(item,index) in addBelongList" :key="index" :label="item.name" :value="item.id"></el-option>
                </el-select>
              </el-form-item> -->
  ...
  export default {
    ...
    methods: {
      ...
      getSelectList(){
        ...
        this.$ajax.get("/recovery/user/roleList",{
          params:{
            type: sessionStorage.getItem('userType')
          }
        }).then(function(response){
        ...
        this.$ajax.get("/recovery/user/userDetail",{
          ...
          if(response.data.status == 260){
            _this.gsName = response.data.unit.name
          }else{
      ...
      getData(){
        ...
        var type = '用户';
        if(sessionStorage.getItem('userType') == '医疗机构' || sessionStorage.getItem('userType') == '回收利用企业' || sessionStorage.getItem('userType') == '再利用企业' || sessionStorage.getItem('userType') == '回收物流'){
          type = null;
        }
